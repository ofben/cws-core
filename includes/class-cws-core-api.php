...
class CWS_Core_API {
    ...
    public function format_job_data( $job ) {
        return array(
            ...
            'entity_status'   => isset( $job['entity_status'] ) ? sanitize_text_field( $job['entity_status'] ) : '',
            'primary_city'    => isset( $job['primary_city'] ) ? sanitize_text_field( $job['primary_city'] ) : '',
            'primary_state'   => isset( $job['primary_state'] ) ? sanitize_text_field( $job['primary_state'] ) : '',
            'primary_country' => isset( $job['primary_country'] ) ? sanitize_text_field( $job['primary_country'] ) : '',
            'primary_location' => isset( $job['primary_location'] ) ? $job['primary_location'] : array(),
            'primary_category' => isset( $job['primary_category'] ) ? sanitize_text_field( $job['primary_category'] ) : '',
            'department'      => isset( $job['department'] ) ? sanitize_text_field( $job['department'] ) : '',
            'industry'        => isset( $job['industry'] ) ? sanitize_text_field( $job['industry'] ) : '',
            'function'        => isset( $job['function'] ) ? sanitize_text_field( $job['function'] ) : '',
            ...
            'seo_url'         => isset( $job['seo_url'] ) ? esc_url( $job['seo_url'] ) : '',
            ...
            'permalink'       => array(
                'relative' => '/' . get_option( 'cws_core_job_slug', 'job' ) . '/' . (int) $job['id'] . '/' . ( ! empty( $job['title'] ) ? sanitize_title( $job['title'] ) . '/' : '' ),
                'absolute' => home_url( '/' . get_option( 'cws_core_job_slug', 'job' ) . '/' . (int) $job['id'] . '/' . ( ! empty( $job['title'] ) ? sanitize_title( $job['title'] ) . '/' : '' ) ),
            ),
